...
test.describe('Anybody Editing Plugin', () => {
  test('plugin is active in WordPress admin', async ({ page }) => {
    ...
    const pluginName = page.locator('td.plugin-title strong:has-text("Anybody Editing")');
  ...
  test('settings page exists', async ({ page }) => {
    ...
    await expect(page.locator('h1:has-text("Anybody Editing Settings")')).toBeVisible({ timeout: 10000 });
    await expect(page.locator('label[for="max_upload_size"]')).toBeVisible();
  });
  ...
  test('media library filter dropdown exists', async ({ page }) => {
    ...
    const filterDropdown = page.locator('select[name="anybody_editing_filter"]');
  ...
  test('meta box appears on classic editor', async ({ page }) => {
    ...
    if (isEditor > 0) {
      ...
      const metaBox = page.locator('#anybody-editing-meta-box, :text("Public Editing")');
  ...
  test('REST API post update requires editing enabled', async ({ request }) => {
    // Try to update a post without editing enabled - should get 403
    const response = await request.post('/wp-json/anybody-editing/v1/posts/1', {
      data: { title: 'Test' },
  ...
  test('REST API upload endpoint exists', async ({ request }) => {
    ...
    const response = await request.post('/wp-json/anybody-editing/v1/upload', {
      data: { post_id: 1 },
    });
...
test.describe('Plugin Configuration', () => {
  test('default settings values are correct', async ({ page }) => {
    ...
    const maxSizeInput = page.locator('input[name="anybody_editing_max_upload_size"]');
    await expect(maxSizeInput).toHaveValue('2');

    // Check all image type checkboxes exist
    const jpgCheckbox = page.locator('input[name="anybody_editing_allowed_types[]"][value="jpg"]');
    const pngCheckbox = page.locator('input[name="anybody_editing_allowed_types[]"][value="png"]');
    const gifCheckbox = page.locator('input[name="anybody_editing_allowed_types[]"][value="gif"]');
    const webpCheckbox = page.locator('input[name="anybody_editing_allowed_types[]"][value="webp"]');
...
test.describe('Security', () => {
  test('REST API validates post existence', async ({ request }) => {
    ...
    const response = await request.post('/wp-json/anybody-editing/v1/posts/99999', {
      data: { title: 'Test' },
      headers: { 'Content-Type': 'application/json' },
    });
